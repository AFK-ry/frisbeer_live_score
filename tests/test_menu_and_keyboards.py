...
class KeyboardTests(unittest.TestCase):
    ...
    def test_game_list_empty_and_sorted(self):
        self.assertEqual(
            keyboards.game_list_keyboard([]).inline_keyboard[-1][0].callback_data,
            "main",
        )
...
class MenuTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_cancel_with_and_without_state(self):
        for user_data in ({"create_game": {}}, {}):
            update = callback_update()
            with patch("handlers.menu.main_menu", AsyncMock()) as mocked:
                await menu.cancel_game_creation(
                    update, SimpleNamespace(user_data=user_data)
                )
                mocked.assert_awaited_once()
    ...
    async def test_handle_message_ignores_false_state(self):
        update = SimpleNamespace(
            message=SimpleNamespace(text="aa,bb,cc", reply_text=AsyncMock())
        )
        await menu.handle_message(
            update, SimpleNamespace(user_data={"create_game": {}})
        )
    ...
    async def test_two_stage_game_creation_with_defaults_and_custom_values(self):
        ...
        first = SimpleNamespace(
            message=SimpleNamespace(text="Ann,Bob,Cara", reply_text=AsyncMock())
        )
        ...
        second = SimpleNamespace(
            ...
            effective_user=SimpleNamespace(
                id=1, username=None, first_name="T", last_name="U"
            ),
        )
    ...
    async def test_stage_two_defaults(self):
        ...
        update = SimpleNamespace(
            ...
            effective_user=SimpleNamespace(
                id=1, username="tester", first_name="T", last_name="U"
            ),
        )
        await menu.handle_message(update, context)
