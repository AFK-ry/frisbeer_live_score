...
class SerializationTests(unittest.TestCase):
    def test_all_actions_round_trip(self):
        actions = [
            SwitchSides(),
            AssignKnocks("team1", "A", [("team2", 0, "k")]),
            StartGame(),
            StartRound(2),
            EndRound("team1"),
        ]
...
class MainTests(unittest.TestCase):
    def test_register_handlers(self):
        ...
        self.assertEqual(
            app.add_handler.call_count,
            len(main.COMMAND_HANDLERS) + len(main.CALLBACK_HANDLERS) + 1,
        )

    def test_setup_infrastructure(self):
        app = SimpleNamespace(bot_data={}, bot=object())
        with patch("main.Storage", return_value="storage"), patch(
            "main.Broadcaster", return_value="broadcaster"
        ):
            main.setup_infrastructure(app, 123)
        self.assertEqual(
            app.bot_data, {"storage": "storage", "broadcaster": "broadcaster"}
        )

    def test_main_builds_and_runs_application(self):
        app = SimpleNamespace(run_polling=Mock())
        builder = SimpleNamespace(
            token=Mock(return_value=SimpleNamespace(build=Mock(return_value=app)))
        )
        with patch("main.Application.builder", return_value=builder), patch(
            "main.setup_infrastructure"
        ) as setup, patch("main.register_handlers") as register:
            main.main("token", 123)
    ...
    def test_script_entrypoint(self):
        app = SimpleNamespace(
            bot_data={}, bot=object(), add_handler=Mock(), run_polling=Mock()
        )
        builder = SimpleNamespace(
            token=Mock(return_value=SimpleNamespace(build=Mock(return_value=app)))
        )
        ...
        def getenv(name, default=None):
            return {"TOKEN": "token", "BROADCAST_CHAT_ID": "123"}.get(
                name, real_getenv(name, default)
            )

        with patch("os.getenv", side_effect=getenv), patch("dotenv.load_dotenv"), patch(
            "telegram.ext.Application.builder", return_value=builder
        ), patch(
            "infrastructure.storage.sqlite.Storage", return_value="storage"
        ), patch(
            "infrastructure.telegram.broadcaster.Broadcaster",
